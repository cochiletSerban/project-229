...
let sendStateToBoard = function sendStateToBoard(
  state,
  boardComponents,
  board
) {
...
let applyStateToLocal = function applyStateToLocale(state, boardComponents) {
  boardComponents.strip2
    .intensity(state.state229.state2.brightness)
    .color(state.state229.state2.color);
  boardComponents.strip22
    .intensity(state.state229.state22.brightness)
    .color(state.state229.state22.color);
  boardComponents.strip229
    .intensity(state.state229.state229.brightness)
    .color(state.state229.state229.color);
...
let applyStateToRemote = function applyStateToRemote(state, board) {
  ...
  board.io.i2cWrite(0x08, [
    mode,
    roofColors.red,
    roofColors.green,
    roofColors.blue,
    roofColors.brightness,
    roofColors.animation,
    state.wallGreen,
    state.wallBlue,
  ]);
};
